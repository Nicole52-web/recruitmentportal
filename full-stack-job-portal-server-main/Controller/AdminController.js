...
exports.getAllInfo = async (req, res, next) => {
    try {
        ...
        const urgentJobs = await JobModel.find({ jobStatus: "urgent" });
        const openJobs = await JobModel.find({ jobStatus: "open" });
        const closedJobs = await JobModel.find({ jobStatus: "closed" });
        ...
        res.status(200).json({
            ...
            job: jobs?.length || 0,
            urgent: urgentJobs?.length || 0,
            open: openJobs?.length || 0,
            closed: closedJobs?.length || 0,
        });
    } catch (error) {
